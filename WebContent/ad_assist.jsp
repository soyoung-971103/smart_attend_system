								...
								<table class="table table-bordered table-sm table-hover" style="width:100%" id="example">
									...
									<tbody>
										<c:forEach var="item" items="${alMember}">
											<tr>
											<td>${item.getDepart_id().getName()}</td>
											<td>${item.getName() }</td>
											<td>${item.getTel() }</td>
											<td>${item.getPhone() }</td>
											<td>${item.getEmail() }</td>
											<td>
												<a href="assist-info.do?id=${item.id }" class="btn btn-xs btn-outline-primary">수정</a>
												<a href="assist-delete.do?id=${item.id }" class="btn btn-xs btn-outline-danger" onClick="return confirm('삭제할까요 ?');">삭제</a>
											</td>
											</tr>
									</c:forEach>
									</tbody>
